...
for (c=0; c<brickColumnCount; c++) {
  ...
  for (r=0; r<brickRowCount; r++) {
    bricks[c][r] = { x: 0, y: 0, status: 1, color: "#0095DD", colorSet: false, moreBalls: false, extraLife: false, grenade: false };
  }
}
...
//  yellow block gives 2 more balls
  ...
  if (!brickYellow.colorSet) {
    ...
    brickYellow.status = 3;
    brickYellow.colorSet = true;
  }

  // green block gives extra life
  let rowGreen = randomRow();
  let columnGreen = randomColumn();
  ...
  let rowRed = randomRow();
  let columnRed = randomColumn();
